...
int cariProduk(int idCari) { // ini buat nyari berdasarkan ID pakai Binary Search, return indeks atau -1 kalau gak ketemu
    // binary search bisa dipake di sini karena ID produk selalu urut ascending (auto increment dari idBerikutnya)
    int low  = 0;
    ...
    while (low <= high) {
        ...
        if (daftarProduk[mid].idProduk == idCari) {
            return mid; // ketemu, langsung return indeksnya
    ...
    return -1; // gak ketemu sama sekali
}
...
void cariProduk(string keyword) { // ini wrapper buat manggil linear search dengan pointer, biar pemanggilan dari luar tetap sama
