...
struct Kategori { // struct buat kategori tar disimpan di dalam struct produk (jadi nested struct)
    string namaKategori;
};

struct Produk { // struct buat produk pakai nested struct Kategori 
    int      idProduk;
    string   namaProduk;
    Kategori kategori;
    int      hargaProduk;
...
struct Pengguna { // struct buat pengguna nanti role isinya admin atau user
    string username;
    string password;
    string role;
};

Produk   daftarProduk[MAX_PRODUK]; // variabel globalnya
int      jumlahProduk = 0;
int      idBerikutnya = 1;
...
int  cariProduk(int idCari);
void cariProduk(string keyword);

bool cekLogin(string usernameInput, string passwordInput, string &usernameAktif, string &roleAktif); // & sebagai parameter fungsinya
void prosesRegister();
...
int inputAngka() { // fix bug infinity loop kalau input bukan angka
    int nilai;
    while (!(cin >> nilai)) {
        cin.clear();
        cin.ignore(1000, '\n');
...
void cetakBarisProduk(Produk *produkPtr) { // * sebagai parameter fungsi + implementasi pointer pada struct
    ...
    string hargaStr = "Rp " + to_string(produkPtr->hargaProduk); // akses harga lewat pointer
...
int hitungTotalBelanja(int idKeranjang[], int jumlahKeranjang[], int isiKeranjang, int indeks) {
    ...
    return subtotal + hitungTotalBelanja(idKeranjang, jumlahKeranjang, isiKeranjang, indeks + 1); // ini rekursif, jadi subtotal item ini kita tambahkan sama total semua item setelahnya
}

int cariProduk(int idCari) { // overload 1 buat nyari berdasarkan ID
    for (int i = 0; i < jumlahProduk; i++) {
...
void cariProduk(string keyword) { // overload 2
    for (int j = 0; keyword[j] != '\0'; j++) // kita ubah keyword nya ke huruf kecil semua biar nda sensitive
    ...
    for (int i = 0; i < jumlahProduk; i++) {
        string namaCek = daftarProduk[i].namaProduk; // kita ubah nama produk nya ke lowercase biar bisa dicek
        for (int j = 0; namaCek[j] != '\0'; j++)
            if (namaCek[j] >= 'A' && namaCek[j] <= 'Z') namaCek[j] += 32;

        int panjangKw = 0, panjangNm = 0; // kita hitung panjang keyword dan namanya
        for (int j = 0; keyword[j] != '\0'; j++) panjangKw++;
        for (int j = 0; namaCek[j] != '\0'; j++) panjangNm++;
        bool cocok = false; // kita cek keywordnya ada di dalam nama apa nda
...
bool cekLogin(string usernameInput, string passwordInput, string &usernameAktif, string &roleAktif) { // ini buat ngecek loginnya, & dipakai biar nilai usernameAktif dan roleAktif berubah di luar fungsi
    for (int i = 0; i < jumlahPengguna; i++) {
        if (usernameInput == daftarPengguna[i].username &&
            passwordInput == daftarPengguna[i].password) {
            usernameAktif = daftarPengguna[i].username; // langsung ubah nilai aslinya lewat referensi
            roleAktif     = daftarPengguna[i].role;     // langsung ubah nilai aslinya lewat referensi
            return true;
        }
    }
    return false;
...
void prosesRegister() { // prosedur daftar akun baru tar langsung jadi user otomatis
    ...
    bool sudahAda = false; // cek dulu username nya sudah dipakai belum
    for (int i = 0; i < jumlahPengguna; i++) {
...
void lihatSemuaProduk() { // prosedur tampilkan semua produk
    cout << "\n--- DAFTAR SEMUA PRODUK ---" << endl;
    cetakTabelProduk();
}

void tambahProduk() { // prosedur tambah produk baru ke array
    ...
    do {  // stok nya gak boleh negatif, kalau salah tar disuruh input ulang
        cout << "Stok        : ";
...
void updateProduk() { // prosedur update salah satu field produk berdasarkan ID
    cout << "\n--- UPDATE PRODUK ---" << endl;
    cetakTabelProduk();
    ...
    int indeks = cariProduk(idUpdate); 
    if (indeks == -1) {
...
void hapusProduk() { // prosedur hapus produk berdasarkan ID nya
    ...
    if (konfirmasi == "y" || konfirmasi == "Y") {
        for (int i = indeks; i < jumlahProduk - 1; i++) {  // kita geser semua elemennya ke kiri trus kita mulai dari posisi yang dihapus
...
void cariProdukAdmin() { // prosedur cari produk di menu admin
    cout << "\n--- CARI PRODUK ---" << endl;
    cout << "Kata kunci: ";
    string keyword;
    getline(cin, keyword);
    cariProduk(keyword); // ini buat panggil overload string
}

void menuAdmin(string usernameAktif) { // ini prosedur menu admin jadi tar looping trus sampai milih logout
    int pilihanAdmin;
    do {
        cout << "\n|==================================================|" << endl;
        ...
        cout << "|==================================================|" << endl;
        cout << "Pilih menu (1-6): ";
        pilihanAdmin = inputAngka(); // fix bug: pakai inputAngka() biar tidak infinite loop
...
void beliProduk(string usernameAktif) { // prosedur proses belanjanya
    cout << "\n--- BELI PRODUK ---" << endl;
    ...
    do {
        ...
        if (isiKeranjang > 0) { // kita tampilkan isi keranjang kalau sudah ada barangnya
        ...
        int idBeli = inputAngka(); // fix bug: pakai inputAngka()
        ...
        if (idBeli == 0) {
            ...
            if (indeks == -1) {
                ...
                int jumlahBeli; // ini buat input jumlahnya dan ntar gabisa lebih dari stok inputnya
                do {
                    cout << " Jumlah  : ";
                    jumlahBeli = inputAngka(); // fix bug: pakai inputAngka()
                    if (jumlahBeli <= 0)
                ...
                bool sudahDiKeranjang = false; // kita cek sudah ada di keranjang atau belum produkya
                ...
                if (!sudahDiKeranjang) { // kalau belum ada ya kita tambahkan sebagai item baru
    ...
    int totalBelanja = hitungTotalBelanja(idKeranjang, jumlahKeranjang, isiKeranjang, 0); // ini buat hitung total pakainya fungsi rekursif 
    ...
    if (konfirmasi == "y" || konfirmasi == "Y") {
        for (int i = 0; i < isiKeranjang; i++) {  // ngurangin stok semua produk di keranjang
            int indeks = cariProduk(idKeranjang[i]);
            if (indeks != -1)
...
void menuUser(string usernameAktif) { // prosedur menu user jadi tar looping trus sampai milih logout
    int pilihanUser;
    do {
        cout << "\n|==================================================|" << endl;
        ...
        cout << "|==================================================|" << endl;
        cout << "Pilih menu (1-4): ";
        pilihanUser = inputAngka(); // fix bugnya pakai inputAngka() biar gak infinite loop
...
int main() { // main menu nya enih
    daftarPengguna[0] = {"syarifah", "2509106007", "admin"}; // adminnya ya saya
    jumlahPengguna = 1;
    jumlahProduk = 0; // produk awalnya kosong dulu
    idBerikutnya = 1;
    ...
    do {
        ...
        cout << "|============================================================|" << endl;
        cout << "Pilih menu (1-3): ";
        pilihanAwal = inputAngka(); // fix bug nya ganti pakai inputAngka() biar gak infinite loop
        ...
        if (pilihanAwal == 1) {
            ...
            do { // maksimal 3 kali percobaan login
                kesempatan++;
                ...
                loginOK = cekLogin(usernameInput, passwordInput, usernameAktif, roleAktif); // fungsi cekLogin pakai & buat ubah nilai usernameAktif dan roleAktif langsung

                if (!loginOK)
                    cout << "Username/password salah. Sisa kesempatan: " << (3 - kesempatan) << "x" << endl;
