...
Produk   daftarProduk[MAX_PRODUK]; // variabel global 
int      jumlahProduk = 0;
int      idBerikutnya = 1;
...
void cetakBarisProduk(Produk produk);
...
void cetakBarisProduk(Produk produk) { // prosedur buat nyetak baris data produk
    string namaTampil = produk.namaProduk; // potong nama kalau lebih dari batas kolom biar muat di tabel
    if ((int)namaTampil.size() > LEBAR_NAMA)
        namaTampil = namaTampil.substr(0, LEBAR_NAMA);

    string katTampil = produk.kategori.namaKategori;
    if ((int)katTampil.size() > LEBAR_KAT)
        katTampil = katTampil.substr(0, LEBAR_KAT);
    string hargaStr = "Rp " + to_string(produk.hargaProduk);

    cout << "| " << left  << setw(LEBAR_ID)        << produk.idProduk
         << " | " << left  << setw(LEBAR_NAMA)      << namaTampil
         << " | " << left  << setw(LEBAR_KAT)       << katTampil
         << " | " << left  << setw(LEBAR_HARGA + 3) << hargaStr  
         << " | " << left  << setw(LEBAR_STOK)      << produk.stokProduk
         << " |" << endl;
}
...
bool cekLogin(string usernameInput, string passwordInput, string &usernameAktif, string &roleAktif) { // ini buat ngecek loginnya 
...
void prosesRegister() { // prosedur daftar akun baru tar langsung jadi user otomatis
    ...
    if (sudahAda) {
        cout << "Username sudah digunakan, coba cari username yang lain." << endl;
        return;
    }
...
void tambahProduk() { // prosedur tambah produk baru ke array
    ...
    cout << "Harga (Rp)  : ";
    cin  >> daftarProduk[indeksBaru].hargaProduk;
    cin.ignore(1000, '\n');

    do {  // stok nya gak boleh negatif, kalau salah tar disuruh input ulang
        cout << "Stok        : ";
        cin  >> daftarProduk[indeksBaru].stokProduk;
        cin.ignore(1000, '\n');
    ...
    do {
        ...
        if (pilihanAwal == 1) {
            ...
            do { // maksimal 3 kali percobaan login
                ...
                loginOK = cekLogin(usernameInput, passwordInput, usernameAktif, roleAktif); // fungsi cekLogin 
